...
page's job — `.contextmenu-overlay:not([tc-expanded]) { display: none }` in
`styles/demos.css`. Left live it swallowed the right-click that opens the menu,
and under `glass`, which gives overlays a `backdrop-filter`, it blurred that
demo permanently.
...
`{ fetch(request) }` — with a named `handleRequest(request)` beside it. Neither
listens on a port, serves an asset, or knows anything about a host.
...
The preset is **`vercel`**, so `pnpm build` writes `.vercel/output` — the Build
Output API, which the platform consumes with no framework detection: the static
build under `static/`, the SSR route as a function, and the cache headers
...
`NITRO_PRESET` overrides the config, which is all `build:node` does — the same
route, the same bundle, a different wrapper. Changing host is that one word:
`netlify`, `cloudflare-module`, `deno-deploy`, and the rest.
...
`pnpm test:prod` builds the Node output and runs the same suite against it.
Worth doing before a release: the failure this site has been bitten by most —
a subtree that never hydrates — is loud in dev and completely silent in a
production build, so the only thing that catches it there is a test that clicks
something and expects it to respond.
...
`ColorSchemeProvider` — originally because that provider could not hydrate at
all (fixed upstream in `solid-use@1.0.0-next.3`, see
[docs/color-scheme-hydration.md](docs/color-scheme-hydration.md)), and now for
